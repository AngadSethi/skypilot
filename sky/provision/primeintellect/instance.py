    ...
    for _ in range(to_start_count):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f'Failed to parse vCPUs/memory from instance type {instance_type}: {e}'
                )
...
def terminate_instances(
    ...
    if not non_terminated_instances:
        logger.info(
            f'All instances for cluster {cluster_name_on_cloud} are already terminated or being deleted'
        )
        return
    ...
    logger.info(
        f'Terminating {len(non_terminated_instances)} instances for cluster {cluster_name_on_cloud}: {instance_names}'
    )
    ...
    for inst_id, inst in non_terminated_instances.items():
        logger.debug(
            f'Terminating instance {inst_id} (status: {inst["status"]})')
        ...
        try:
            ...
            logger.info(
                f'Successfully initiated termination of instance {inst_id} ({inst["name"]})'
            )
        except Exception as e:  # pylint: disable=broad-except
            with ux_utils.print_exception_no_traceback():
                raise RuntimeError(
    ...
    if not terminated_instances:
        logger.info(
            'No instances were terminated (worker_only=True and only head node found)'
        )
        return
    ...
    for _ in range(MAX_POLLS_FOR_UP_OR_STOP):
        ...
        logger.info(
            f'Waiting for termination... {len(still_exist)} instances still exist: {remaining_statuses}'
        )
        time.sleep(POLL_INTERVAL)
    else:
        # Timeout reached
        ...
        if still_exist:
            logger.warning(
                f'Timeout reached. {len(still_exist)} instances may still be terminating: {still_exist}'
            )
        else:
            logger.info('All instances have been successfully terminated')
...
def get_cluster_info(
    ...
    for instance_id in running_instances.keys():
        ...
        ssh_connection = running_instances[instance_id]['sshConnection']
        if isinstance(ssh_connection, str) and ' -p ' in ssh_connection:
            ssh_port = int(ssh_connection.split(' -p ')[1].strip())
        else:
            ssh_port = 22
